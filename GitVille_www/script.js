  ...
  try {
    ...
    if (roadsRes && roadsRes.ok) {
      try {
        ...
        console.log("No roads found or invalid JSON");
      }
    }
    ...
    houses = [
      { x: 0, y: 0, color: "#ff6b6b", hoverAnim: 0, username: "Error" },
    ];
    alert(
    ...
    for (let gx = startX; gx <= endX; gx++) {
      ...
      if (roads.has(tileKey)) {
        ...
        if (noise < 0.6) ctx.fillStyle = colors.grassBase; // Base
          ...
          ctx.fillStyle = colors.grassDark; // Slightly Darker
        ...
        if (noise > 0.7) {
          ...
          if (decType < 6) {
            ...
            ctx.beginPath();
            ctx.moveTo(tx, ty);
            ctx.lineTo(tx - 3 + sway, ty - 4);
            ctx.moveTo(tx, ty);
            ctx.lineTo(tx + 2 + sway, ty - 5);
            ctx.stroke();
          } else if (decType < 9) {
            // Flowers (Simple dots)
            const colors = [
...
function drawRoadTile(gx, gy, pos) {
  ...
  if (hasN) {
    ctx.beginPath();
    ctx.moveTo(g2w(-w, -0.5).x, g2w(-w, -0.5).y);
    ctx.lineTo(g2w(-w, -w).x, g2w(-w, -w).y);
    ctx.stroke();
    ctx.beginPath();
    ctx.moveTo(g2w(w, -0.5).x, g2w(w, -0.5).y);
    ctx.lineTo(g2w(w, -w).x, g2w(w, -w).y);
    ...
    ctx.beginPath();
    ctx.moveTo(g2w(-w, -w).x, g2w(-w, -w).y);
    ctx.lineTo(g2w(w, -w).x, g2w(w, -w).y);
    ctx.stroke();
  }
  // S
  ...
  if (hasE) {
    ctx.beginPath();
    ctx.moveTo(g2w(0.5, -w).x, g2w(0.5, -w).y);
    ctx.lineTo(g2w(w, -w).x, g2w(w, -w).y);
    ctx.stroke();
    ctx.beginPath();
    ctx.moveTo(g2w(0.5, w).x, g2w(0.5, w).y);
...
function drawHouse(
  ...
  ctx.lineTo(
    toScreen(hw - trimW, -hd, wallHeight).x,
    toScreen(hw - trimW, -hd, wallHeight).y
  ); // Up
  ...
  if (wStyle === -1) {
    ...
    function drawPlank(zPos, width, angle) {
      ...
      const nail = toScreen(
        cx,
        cy + p1.y + (p2.y - p1.y) * 0.1,
        cz + p1.z + (p2.z - p1.z) * 0.1
      );
      ctx.beginPath();
      ctx.arc(nail.x, nail.y, 0.5, 0, Math.PI * 2);
      ctx.fill();
      const nail2 = toScreen(
        cx,
        cy + p2.y - (p2.y - p1.y) * 0.1,
        cz + p2.z - (p2.z - p1.z) * 0.1
      );
      ctx.beginPath();
      ctx.arc(nail2.x, nail2.y, 0.5, 0, Math.PI * 2);
      ctx.fill();
    }
  ...
  ctx.lineTo(
    toScreen(-hw, hd - trimW, wallHeight).x,
    toScreen(-hw, hd - trimW, wallHeight).y
  );
  ctx.lineTo(t4.x, t4.y);
  ctx.fill();
  ...
  if (dStyle === -1) {
    ...
    ctx.beginPath();
    ctx.moveTo(d_ml.x, d_ml.y);
    ctx.lineTo(d_mr.x, d_mr.y);
    ctx.stroke();
  } else if (dStyle === 0) {
  ...
  const styleIndex =
    roofStyle !== undefined ? roofStyle : Math.abs(gx + gy) % 4;
  ...
  if (cStyle !== 0) {
    ...
    if (doCap) {
      ...
      ctx.beginPath();
      ctx.moveTo(c_fl_bot.x, c_fl_bot.y);
      ctx.lineTo(c_fr_bot.x, c_fr_bot.y);
      ctx.lineTo(c_fr_top.x, c_fr_top.y);
      ctx.lineTo(c_fl_top.x, c_fl_top.y);
      ctx.fill();
      ctx.stroke();
      ...
      ctx.beginPath();
      ctx.moveTo(c_fr_bot.x, c_fr_bot.y);
      ctx.lineTo(c_br_bot.x, c_br_bot.y);
      ctx.lineTo(c_br_top.x, c_br_top.y);
      ctx.lineTo(c_fr_top.x, c_fr_top.y);
      ctx.fill();
      ctx.stroke();
      ...
      ctx.beginPath();
      ctx.moveTo(c_fl_top.x, c_fl_top.y);
      ctx.lineTo(c_fr_top.x, c_fr_top.y);
      ctx.lineTo(c_br_top.x, c_br_top.y);
      ctx.lineTo(c_bl_top.x, c_bl_top.y);
      ctx.closePath();
      ctx.fill();
    } else {
      // Standard Top Face
    ...
    if (cStyle === 1) {
      ...
      ctx.beginPath();
      ctx.moveTo(fl_fl_bot.x, fl_fl_bot.y);
      ctx.lineTo(fl_fr_bot.x, fl_fr_bot.y);
      ctx.lineTo(fl_fr_top.x, fl_fr_top.y);
      ctx.lineTo(fl_fl_top.x, fl_fl_top.y);
      ctx.fill();

      // Side
      ctx.fillStyle = adjustColor(fColor, -15);
      ctx.beginPath();
      ctx.moveTo(fl_fr_bot.x, fl_fr_bot.y);
      ctx.lineTo(fl_br_bot.x, fl_br_bot.y);
      ctx.lineTo(fl_br_top.x, fl_br_top.y);
      ctx.lineTo(fl_fr_top.x, fl_fr_top.y);
      ctx.fill();
      ...
      ctx.fillStyle = "#2d3436";
      ctx.beginPath();
      ctx.moveTo(fl_fl_top.x, fl_fl_top.y);
      ctx.lineTo(fl_fr_top.x, fl_fr_top.y);
      ctx.lineTo(fl_br_top.x, fl_br_top.y);
      ctx.lineTo(fl_bl_top.x, fl_bl_top.y);
      ctx.fill();
    ...
    if (cStyle === 3) {
      ...
      [pot1Pos, pot2Pos].forEach((pos) => {
        ...
        ctx.moveTo(b_l.x, b_l.y);
        ctx.lineTo(b_r.x, b_r.y);
        ctx.lineTo(t_r.x, t_r.y);
        ctx.lineTo(t_l.x, t_l.y);
        ctx.fill();
